...
class UnifiedDocumentApp(tk.Tk):
    ...
    def start_monitoring(self):
        ...
        self.observer.start()
        self.status_label.config(text=f"Monitoring started for {directory}")
...
class FileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if not event.is_directory:
            ...
            self.app.file_records.append(event.src_path)
            ...
            if event.src_path.lower().endswith(".pdf"):
                ...
                threading.Thread(target=convert_scanned_pdf_to_ocr, args=(event.src_path, output_path)).start()
                LOGGER.info(f"Started conversion for new PDF: {event.src_path}")

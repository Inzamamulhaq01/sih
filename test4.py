...
class UnifiedDocumentApp(tk.Tk):
    ...
    def convert_pdf(self, doc_path, output_path):
        try:
            ...
            self.after(0, lambda: self.status_label.config(text=f"Status: Conversion completed for {doc_path}"))
        except Exception as e:
            self.after(0, lambda: self.status_label.config(text=f"Status: Error - {str(e)}"))
        finally:
            self.after(0, self.progress_bar.stop)
            self.after(0, self.refresh_file_list)  # Refresh the file list after conversion

    def convert_docx(self, doc_path, output_path):
        try:
            convert_docx_to_searchable(doc_path, output_path)
            self.after(0, lambda: self.status_label.config(text=f"Searchable DOCX saved as: {output_path}"))
        except Exception as e:
            self.after(0, lambda: self.status_label.config(text=f"Status: Error - {str(e)}"))
        finally:
            self.after(0, self.progress_bar.stop)
...
class FileMonitorApp:
    ...
    def start_monitoring(self):
        ...
        self.root.refresh_file_list()  # Refresh file list when monitoring starts
    ...
    def stop_monitoring(self):
        if self.observer:
            ...
            self.root.refresh_file_list()  # Refresh file list when monitoring stops
